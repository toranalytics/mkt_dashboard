...
def get_creative_details(ad_id, ver, token):
    ...
    try:
        ...
        if creative_id:
            ...
            # --- 유형 판별 로직 개선 ---
            # 1. 명시적 VIDEO 타입 또는 video_id가 있는 경우
            ...
            # 2. 명시적 PHOTO 타입 또는 image_url이 있는 경우
            ...
            # 3. SHARE 타입 처리 (개선된 로직)
            elif object_type == 'SHARE':
                # asset_feed_spec에 videos가 있으면 동영상으로 판단
                if videos_from_feed:
                    creative_details['content_type'] = '동영상'
                ...
                # 그 외의 경우 썸네일 유무로 판단
                    ...
                    # effective_object_story_id가 있으면 인스타그램 링크 시도
...
def fetch_creatives_parallel(ad_data, ver, token, max_workers=10):
    """
    ThreadPoolExecutor를 사용하여 여러 광고의 크리에이티브 정보를 병렬로 가져옵니다. (이전과 동일)
    """
...
def fetch_and_format_facebook_ads_data(start_date, end_date, ver, account, token):
    """
    메인 함수: 데이터 가져오기, 처리, HTML 생성 (이전과 동일, get_creative_details 호출만 변경됨)
    """
    ...
    for record in records:
        ...
        if actions and isinstance(actions, list):
            for action in actions:
                ...
                try: value = int(action.get("value", 0))
                except (ValueError, TypeError): value = 0
                if action_type == "link_click": link_clicks += value
                elif action_type == "purchase" or action_type.startswith("omni_purchase"): purchase_count += value
    ...
    # 2) 크리에이티브 정보 병렬 가져오기 (수정된 get_creative_details 사용)
    fetch_creatives_parallel(ad_data, ver, token, max_workers=10)

    # 3) DataFrame 변환 및 정리 (이전과 동일)
    ...
    for index, row in df_sorted.iterrows():
        ...
        if display_url:
            ...
            if target_url: content_tag = f'<a href="{target_url}" target="_blank">{img_tag}</a>'
